...
def getWordScore(word, n):
    ...
    if n == len(word):
        bonus = 50
    for i in list(word):
        score +=  int(SCRABBLE_LETTER_VALUES[i])
...
def isValidWord(word, hand, wordList):
    ...
    """
    wordCount = getFrequencyDict(list(word))
    
    if word in wordList:
        for letter in wordCount.keys():
            if wordCount.get(letter) <= hand.get(letter,0):
                continue
            else:
                return False
                break               
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            print(word, ' get ',getWordScore(word, n),' points. Total: ', totalScore,' points')
            print('')
...
def playGame(wordList):
    ...
    while n >= 0:
        ...
        if command == 'n':
            ...
            if hand == {}:
                print('You hrave not played a hand yet. Please play a new hand first!')
            else:
                playHand(hand,wordList,n)
        elif command == 'e':
            break
